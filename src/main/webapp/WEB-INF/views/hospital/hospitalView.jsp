...
<script>
    function hosLikeButton() {
        if (${empty sessionScope.id}) {
            ...
            var hosLikeData = 'hos_no='+${hospital.hos_no}+
            '&id=' + "${sessionScope.id}";
...
<section class="py-5">
    <div class="container-body">
            ...
            <tr>
                <c:if test="${hospital.hos_24 == 'Y' || hospital.hos_holiday == 'Y' || hospital.hos_parking == 'Y'}">
                    ...
                    <td>
                        ...
                        <div class="info">
                            <c:if test="${not empty hospital.hos_24 && hospital.hos_24 == 'Y'}"><input type="button"
                                                                                                       class="btn btn-warning"
                                                                                                       value="24시"/> </c:if>
                            <c:if test="${not empty hospital.hos_holiday && hospital.hos_holiday == 'Y'}"><input
                                    type="button" class="btn btn-primary" value="연중무휴"/></c:if>
                            <c:if test="${not empty hospital.hos_parking && hospital.hos_parking == 'Y'}"><input
                                    type="button" class="btn btn-info" value="주차가능"/></c:if>
                        </div>
                    </td>
        ...
        <form name="hos_form" id="hos_form">
            ...
            <c:if test="${sessionScope.id != null}">
                <textarea class="form-control" name="hosReply_content" rows="2"></textarea>
                <div class="body-menu">
                    <input type="button" value="확인" id="hosReInsert" class="btn btn-outline-secondary">
                </div>
            </c:if>
        </form>
    </div>
